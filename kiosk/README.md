...
```
환자 폰/태블릿 ──▶ /intake?k=<슬러그>&c=<방문 코드>   (B: 환자 등록형 개인 링크)
               ──▶ /intake?k=<슬러그>              (A: 원내 고정 QR, 무코드)
                     │  (고지·동의 → 환자 정보 → AI 문진 → 완료)
                     ▼
...
## 접수 흐름 세 가지 (2026-08, 0019)

방문 코드 자체는 그대로입니다. 바뀐 것은 **코드를 누가 어떻게 받아 드는가** 입니다.
접수대 건너에서 7자를 불러주는 흐름이 실제로 실패하던 지점이었습니다.

### B) 환자 등록형 개인 링크 — 기본 경로

데스크톱 dock 의 QR 버튼에서 **환자 이름(필수)·휴대폰(선택)** 을 입력하고
"등록하고 링크 만들기" 를 누릅니다. 그러면

1. `patients` 행이 그 자리에서 만들어지고(발급 의사 소유),
2. 그 환자에게 **묶인** 방문 코드가 발급되고(`issue_patient_visit_code`),
3. 큰 QR + 링크 복사 + (번호가 있으면) 알림톡 발송 버튼이 뜹니다.

환자가 그 링크를 열면 코드 화면 없이 곧바로 동의로 들어가고, **이름 칸이 미리
채워져 있습니다**(고칠 수 있습니다 — 접수 입력이 틀렸다면 본인이 더 정확합니다).
문진이 시작될 때 새 환자 행을 만들지 않고 등록된 그 행에 진료를 붙입니다.

수명은 **24시간**입니다(익명 코드는 30분). 진료 전날 저녁에 보낸 링크가 아침에
죽어 있으면 환자는 자기 탓을 하고 다시 시도하지 않습니다.

> 같은 이름·번호로 다시 등록하면 **새 환자 행이 또 생깁니다.** 일부러 그렇게
> 했습니다: 이름이 같은 다른 사람을 하나로 합치는 실수는 대기목록에서 보이지
> 않고, 중복은 보이며 고칠 수 있습니다. 병합은 두 기록을 사람이 함께 볼 수 있는
> 화면에서 할 결정입니다.

### A) 원내 고정 QR (walk-in) — 폴백

`?k=<슬러그>` 만 있고 `?c=` 가 없는 접속입니다. 대기실 벽에 붙여두는 QR 이고,
**코드 입력 화면을 보여주지 않습니다.** 바로 동의 → 이름·생년월일로 진행합니다.

코드가 없다는 것은 접근 통제가 하나 사라졌다는 뜻이므로 세 가지가 그 자리를
대신합니다(근거: `../supabase/migrations/0019_patient_visit_codes.sql`):

1. **속도 제한** — 출처 주소 + 의사 단위, DB 카운터(`walk_in_intake_attempts`).
   10분 창에서 출처당 5건, 의원당 40건. 주소는 저장하지 않고 HMAC 해시만 남습니다.
2. **`encounters.intake_source = 'walk_in'`** — 접수처가 보증한 문진과 포스터를
   찍은 사람의 문진이 대기목록에서 같은 모습이면 안 됩니다.
3. 담당 의사는 여전히 슬러그로만 정해집니다. 클라이언트는 의사를 지목할 수 없습니다.

만료·오타 `?c=` 로 들어온 경우에는 **왜 안 되는지 한 줄로 알려준 뒤** 이 경로로
이어가게 합니다. 조용히 흘려보내면 환자는 자기가 무엇을 잘못했는지 모릅니다.

### 빠른 코드 (환자 미지정) — 종전 그대로

환자가 이미 접수대 앞에 서 있을 때 쓰는 30분짜리 익명 코드입니다.
`issue_visit_access_code` 는 손대지 않았고, 코드 입력 화면 컴포넌트
(`app/intake/VisitCodeStep.tsx`)도 **지우지 않고 라우팅에서만 뺐습니다** —
무코드 폴백이 현장에서 어떻게 쓰이는지 보기 전까지 되돌릴 수 있어야 합니다.

---

## 알림톡 발송 (Solapi)

**보내는 주체는 이 키오스크 서버입니다.** 데스크톱 앱이 직접 보내면 Solapi
자격증명이 설치본 안에 들어가고, 그러면 빌드를 가진 누구나 우리 계정으로 문자를
보낼 수 있으며 유출돼도 전 설치본을 재배포하기 전에는 회수할 수 없습니다
(A1 이 provider 키를 걷어낸 것과 같은 이유).

데스크톱이 보내는 것은 세 가지뿐입니다: 로그인 세션 JWT, 코드 행 id, 링크.
서버가 (1) JWT 로 사람을 확인하고 (2) 그 사람 소유의 코드인지 DB 에서 확인하고
(3) 링크가 **이 배포 자신의 `/intake`** 인지 확인하고 (4) **전화번호는 DB 에서
읽습니다**. 클라이언트가 보낸 번호로 보내면 (2)의 소유 확인이 아무것도 지키지
못하고, 이 라우트는 우리 발신번호를 빌려주는 스팸 게이트가 됩니다.

### 설정 절차 (사람이 해야 하는 일)

1. **카카오톡 채널 개설** 후 Solapi(<https://solapi.com>)에 채널을 연동합니다.
   연동이 끝나면 발신 프로필 id(`pfId`)가 나옵니다 → `SOLAPI_PF_ID`.
2. **발신번호 등록·인증** (통신자료 제출). 알림톡 규격상 필수입니다 →
   `SOLAPI_SENDER_PHONE`.
3. **알림톡 템플릿 등록 후 카카오 검수 승인.** 승인된 템플릿 id →
   `SOLAPI_TEMPLATE_ID`. 본문 예시(변수 이름이 코드와 **정확히** 같아야 합니다):

   ```
   [#{병원명}] 사전 문진 안내

   #{환자명} 님, 진료 전 문진을 미리 작성해 주세요.
   아래 링크를 눌러 5분 정도면 끝납니다.

   #{문진링크}

   문진 내용은 담당 의사가 진료 전에 확인합니다.
   ```

   변수 세 개: `#{병원명}` · `#{환자명}` · `#{문진링크}`.
   버튼을 넣는다면 웹링크 버튼에 `#{문진링크}` 를 연결하세요.
   *광고성 문구를 넣으면 검수에서 반려됩니다.* 이 메시지는 정보성입니다.
4. **API 키 발급** → `SOLAPI_API_KEY` / `SOLAPI_API_SECRET`.
5. 위 5개(+선택 `SOLAPI_CLINIC_NAME`)를 키오스크 배포의 환경변수에 넣고
   **재배포**합니다.

### 설정하지 않았을 때

라우트가 `{ ok: false, configured: false }` 를 200 으로 돌려주고, 데스크톱
다이얼로그는 "알림톡이 아직 설정되지 않았습니다. 링크를 복사해서 보내 주세요."
를 보여줍니다. **자리표시자 값을 넣지 마세요** — 넣으면 발송이 시도되고 실패가
"환자에게 안내가 갔다" 로 잘못 도착합니다.
...
| `SOLAPI_API_KEY` | | 알림톡 발송. **아래 5개를 다 채워야** 발송이 켜집니다. 하나라도 비면 `/api/notify/alimtalk` 가 `{ configured: false }` 를 돌려주고, 데스크톱은 "링크를 복사해서 보내세요" 로 안내합니다. |
| `SOLAPI_API_SECRET` | | 위와 한 쌍. HMAC 서명에 씁니다. |
| `SOLAPI_PF_ID` | | 카카오 발신 프로필 id (채널을 Solapi 에 연동하면 발급). |
| `SOLAPI_TEMPLATE_ID` | | 승인된 알림톡 템플릿 id. |
| `SOLAPI_SENDER_PHONE` | | Solapi 에 등록·인증된 발신번호. 알림톡 규격상 필수입니다. |
| `SOLAPI_CLINIC_NAME` | | 템플릿의 `#{병원명}` 에 들어갈 값. 미설정 시 `병원`. |
