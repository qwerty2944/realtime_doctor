 ...
 *
 * 실패는 전부 /billing/login 으로 보낸다. 여기서 실패했다는 것은 링크가 오래됐거나
...
export async function GET(req: Request) {
  ...
  if (!check.ok) {
    ...
    return NextResponse.redirect(new URL(withBasePath('/billing/login?next=/billing'), req.url), {
      status: 303
    });
  }
  ...
  return NextResponse.redirect(new URL(withBasePath('/billing'), req.url), { status: 303 });
}
